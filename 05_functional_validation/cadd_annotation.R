...
get_cadd <- function(snp) {
  url <- paste0("https://myvariant.info/v1/variant/", snp,
  ...
  tryCatch({
    ...
    return(data.frame(SNP=snp, CADD_score=NA, Consequence=NA, Gene=NA))
  })
}
...
print(final_results)
fwrite(final_results, "credible_set_CADD_scores_complete.csv")
cat("Saved to credible_set_CADD_scores_complete.csv\n")
